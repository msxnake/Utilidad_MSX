# Programa utilidad MSX versión 0004
...
def process_tga_file(file_path, fila, columna, num_sprites):
    print(f"Procesando archivo TGA: {file_path}")
    process_sprites_atlas(
        file_path=file_path,
        fila=int(fila),  # Convertir texto a número
        columna_inicial=int(columna) - 1,  # Ajustar para empezar desde 0
...
class MainApp(MDApp):
    def build(self):
        ...
        # Campo de texto para "Número de Sprites"
    ...
    def open_file_chooser(self, instance):
        ...
        def on_file_selection(instance, selection):
            if selection:  # Verificar que se seleccionó un archivo
                popup.dismiss()
                # Llamar a la función de procesamiento con los parámetros del usuario
